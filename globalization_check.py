...
GLOBALIZATION_SERVICE='gp-beta'              #'IBM Globalization'
GLOBALIZATION_SERVICE_PLAN='gp-beta-plan'          #'Experimental'
            ...
            if service in line:
                ...
                if (serviceStart > 0) and (serviceEnd > 0):
                    if service in line[serviceStart:serviceEnd]:
                        ...
                        Logger.info("Find serviceName: " + serviceName)
                        if serviceName.find("UCD") > -1: 
                          break
        else:
            continue
...
def getGlobalizationCredentialsFromBoundApp (service=GLOBALIZATION_SERVICE, binding_app=None):
    ...
    if binding_app == None:
        setupSpace = "true" 
        #os.environ.get('SETUP_SERVICE_SPACE')
...
def setenvvariable(key, value, filename):
    keyvalue = 'export %s=%s\n' % (key, value)
    Logger.info(keyvalue)
    filename.write(keyvalue)
...
try:
    ...
    f = open("./setenv_globalization.sh", 'w');
    setenvvariable('GP_INSTANCE_ID', instanceId, f)
    setenvvariable('GP_ENDPOINT', url, f)
    setenvvariable('GP_USER_ID', userId, f)
    setenvvariable('GP_PASSWORD', password, f)
    setenvvariable('GP_DASHBOARD', dashboard, f)
    f.flush()
    f.close()
    #export GP_API_KEY=api_key
	#export GP_ENDPOINT=uri
	#export GP_DASHBOARD=dashboard
	
    #Logger.info("GP_API_KEY:"+api_key)
    #Logger.info("GP_ENDPOINT:"+uri)
    #Logger.info("GP_DASHBOARD:"+dashboard)
